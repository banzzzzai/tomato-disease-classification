...
dataloaders = {
    x: torch.utils.data.DataLoader(
        ...
        shuffle=True, num_workers=os.cpu_count() // 3, pin_memory=True
    )
    for x in ['train', 'val']
}
...
# Обучение модели с AMP
def train_model(model, criterion, optimizer, scheduler, num_epochs=20):
    since = time.time()
    writer = SummaryWriter(log_dir='runs/tomato_amp')
    scaler = GradScaler()
...
if __name__ == "__main__":
    ...
    torch.save(model_ft.state_dict(), 'resnet50_tomato_best_amp.pth')
    print("✅ Модель сохранена в файл resnet50_tomato_best_amp.pth")
